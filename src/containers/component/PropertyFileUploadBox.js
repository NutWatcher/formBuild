...
import './PropertyFileUploadBox.less';

export class ComPropertyFileUploadBox extends Component {
    ...
    componentDidMount(){
        const { uId, dispatchSrc, autoUpload, handleOnChange, picker, fileSingleSizeLimit, accept , value} = this.props;
        ...
        if (value != ""){
            window.lyyState.imageIndex = window.lyyState.imageIndex || 1;
            window.lyyState.imageIndex ++ ;
            let tempIndex = window.lyyState.imageIndex;
            var $li = $(
                    '<div id="temp_uploadImage' + tempIndex + '" class="uploadFileItem">' +
                    '<img style="width: 40px;height: 40px;" src=' + value + '>' +
                    '<a id="remove_temp_uploadImage' + tempIndex + '" class="deleteIcon" ' +
                    'style="background:no-repeat scroll 0 0 #fff;' +
                    'background-image: url(\'' + window.formBuildState.ImageLocation + '/cancel.png\')" title="删除" href="#" ></a>'+
                    '</div>'
                ),
                $img = $li.find('img');
            $("#UpFileList" + picker).empty().append( $li ).append('<div className="cl" style=\'clear:both\'></div>');
            $("#UpFileList" + picker).find('#remove_temp_uploadImage'+ tempIndex).click(function () {
                console.log("remove");
                console.log("id window.lyyState.imageIndex" + tempIndex);
                $("#UpFileList" + picker).find('#temp_uploadImage'+ tempIndex).remove();
                handleOnChange(uId, "");
            } );
    ...
    render() {
        ...
        return (
            <div className = "ComPropertyFileUploadBox">
...
function mapStateToProps(state,props) {
   ...
   // console.log(componentPoint);
    return {
        picker:componentPoint.picker||"picker",
...
const ConnectedComPropertyFileUploadBox = connect(mapStateToProps, mapDispatchToProps)(ComPropertyFileUploadBox);
export default ConnectedComPropertyFileUploadBox